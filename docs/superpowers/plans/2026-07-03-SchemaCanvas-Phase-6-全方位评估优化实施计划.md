# SchemaCanvas Phase 6 全方位评估优化实施计划
...
- [x] 调研：确认 SchemaCanvas（原 drawDB）历史发布版本中 IndexedDB schema 版本号范围（检查 git 历史中 `db.js` 的 `db.version()` 调用记录）。
...
- [x] M17：评估 CollabContext 是否移除或接入。
- [x] M18：CloudDiagrams 路由添加鉴权守卫（云端能力启用时）。
...
完成记录：2026-07-04 已完成 M17 CollabContextProvider 接入评估切片。红灯测试先确认 `Editor.jsx` 未导入和包裹 `CollabContextProvider`，而 Canvas、Diagram、Areas、Notes、Types、Enums 已通过 `useCollab()` 依赖该 Context；复核后选择接入而非删除，因为这些调用用于隔离远端应用状态和发出协作 delta/awareness no-op。修复后 Editor Provider 树在这些消费者外层包裹 `CollabContextProvider`，保持本地无账号模式，不启用实时协作或自动网络动作。验证命令：`npm run test -- src/build/collabProviderBoundary.test.js`、`npm run test`、`npm run e2e`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M18 CloudDiagrams 路由鉴权守卫切片。红灯测试先确认 cloud capability 已启用但 session 为 signed-out 时仍会调用 `listCloudDiagrams`/`listTeams` 并显示空列表；修复后 `CloudDiagrams` 会从 props 或 `ExtensionsContext` 读取 cloud capability、cloud session 和账号动作。未配置云端时继续显示 unavailable；云端已配置但未登录或会话过期时显示 `CloudAccountStatus`，不请求云端图表列表；signed-in/authenticated session 才会加载图表和团队列表。验证命令：`npm run test -- src/pages/CloudDiagrams.test.jsx`、`npm run test`、`npm run e2e`、`npm run accessibility`、`npm run lint`、`npm run build`、`npm run bundle:check`、`git diff --check`、`npm audit --audit-level=high`。

下一项：Phase 6 切片队列已完成；下一轮应先按第 9 节评估 Phase 7 架构重构或新的产品 roadmap，不应提前实施未决产品能力。
...
- [x] 所有 Critical 安全漏洞（C1–C4）已修复并有回归测试。
- [x] API 层有完整错误处理和环境变量守卫。
- [x] nginx 和 Vercel 两侧都有完整安全 header。
- [x] IndexedDB 迁移路径完整或有明确安全性文档。
- [x] DBML 解析器按需加载，路由级懒加载已落地。
- [x] 4 个无翻译页面和 CloudDiagrams 翻译键已补齐。
- [x] validateDiagram 测试覆盖显著提升，覆盖率门禁已配置。
- [x] Context value memoize 缓解已落地。
- [x] 本地无账号模式完整可用。
- [x] 不自动上传图表数据。
- [x] 无新增 high/critical 依赖漏洞。
